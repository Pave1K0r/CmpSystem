...
const handleStatusChange = async (row, newStatus) => {
  try {
    ...
    await updateRepairStatusService(row.id, newStatus)
    row.repairsStatus = newStatus
...
const handleOpenReply = (row) => {
  replyForm.id = row.id
  replyForm.content = row.repairsContent || ''
...
const handleSubmitReply = async () => {
  ...
  await replyRepairService(replyForm.id, replyForm.content)
...
<template>
  <el-card class="repair-card">
    ...
    <el-table v-loading="loading" :data="repairList" style="width: 100%" border>
      ...
      <el-table-column prop="repairsStatus" label="处理状态" width="150" align="center">
        <template #default="{ row }">
